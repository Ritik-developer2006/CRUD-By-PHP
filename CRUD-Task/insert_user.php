...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES["photo"]) && $_FILES["photo"]["error"] == 0) {
        ...
        echo "No photo uploaded or file error.";
        exit;
    }

    $stmt = $conn->prepare("INSERT INTO users (`uid`, `name`, `number`, `email`, `role`, `photo`, `status`) VALUES (?, ?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
        echo "Error preparing the statement.";
        exit;
    }

    $stmt->bind_param("sssssss", $uid, $name, $number, $email, $role, $fileName, $status);
    $stmt->execute();
    ...
    if ($stmt->affected_rows > 0) {
        ...
        header("Location: index.php?error=Insert is Failed!");
        exit;
    }
    $stmt->close();
}
